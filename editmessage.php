...
<body>
    <?php
        ...
        if(!empty($_GET['image_okazja'])){
            $sql = 'INSERT INTO images (nazwa, image_dir) VALUES ("foto", :image_okazja)';
            $sth = $dBase->prepare($sql);
            $sth->bindValue(':image_okazja', ($_GET['image_okazja']), PDO::PARAM_STR);
            $sth->execute();
            $sql = 'SELECT * FROM images ORDER BY id_image DESC LIMIT 1';
            $res = $dBase->query($sql);
            foreach($res as $r){
                $img = $r['id_image'];
            }
        }
        $sql = 'UPDATE okazja set cena_okazja=:cena_okazja, tytul_okazja=:tytul_okazja, opis_okazja=:opis_okazja, link_okazja=:link_okazja, 
        id_image=:image_okazja WHERE id_okazja=:id_okazja';
        $sth = $dBase->prepare($sql);
        $sth->bindValue(':cena_okazja', $_GET['cena_okazja'], PDO::PARAM_INT);
        $sth->bindValue(':tytul_okazja', $_GET['tytul_okazja'], PDO::PARAM_STR);
        $sth->bindValue(':opis_okazja', $_GET['opis_okazja'], PDO::PARAM_STR);
        $sth->bindValue(':link_okazja', $_GET['link_okazja'], PDO::PARAM_STR);
        $sth->bindValue(':image_okazja', (empty($_GET['image_okazja']) ? 1 : $img), PDO::PARAM_STR);
        $sth->bindValue(':id_okazja', $_GET['id_okazja'], PDO::PARAM_INT);
        $sth->execute();
    ?>
    <div class="container-fluid">
        ...
        <div class="row d-flex flex-nowrap bg-dark" id="subnav1">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link" id="sub1" href="?sortby=ile_like">Najlepsze</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="sub2" href="?sortby=data_dodania">Najnowsze</a>
                </li>
            </ul>
        </div>
